---
# the default layout is 'page'
icon: fas fa-info-circle
order: 4
---

I'm huskersec — a security researcher focused on **vulnerability research** and **offensive security**. This site is where I consolidate my writeups, research notes, tooling, and the occasional longer-form post.

## What you'll find here

- **Vulnerability research** — root-cause analysis, CVE writeups, and the path from bug to impact.
- **Offensive security** — tradecraft, technique deep-dives, and notes from engagements (sanitized).
- **Tooling** — small utilities and scripts I build along the way, usually with links to the repos.
- **Notes** — shorter posts: things I learned, papers worth reading, rabbit holes worth sharing.

> Everything here is my own work and opinions, not those of any employer. Research is published in line with coordinated-disclosure norms.
{: .prompt-info }

## Background

<!-- TODO: a few sentences on your background — current role, areas of focus,
     how you got into security. Keep it tight; the resume page carries the detail. -->

## Elsewhere

- GitHub: [@huskersec](https://github.com/huskersec)
- Twitter/X: [@huskersec](https://twitter.com/huskersec)
<!-- - Mastodon / LinkedIn / etc. -->

## Contact

<!-- TODO: add an email or contact method if you want one public.
     For a security blog, consider a dedicated address and a PGP key for
     disclosure correspondence. -->
